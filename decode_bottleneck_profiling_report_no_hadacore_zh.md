# QuaRot Decode Bottleneck Profiling：Unfused INT4 vs Fused Current

日期：2026-07-14

## 實驗定位

本報告比較 `unfused_INT4`、`fused_current` 的 Llama-3.1 8B token-by-token decode。所有 speedup 均以 `unfused_INT4` 為 baseline。`unfused_INT4` 尚不是經完整 QuaRot 權重旋轉與 calibration 的官方 converted model。
...
![Latency vs context](decode_bottleneck_profiling_results/charts_no_hadacore/latency_vs_context_by_batch.png)

![Speedup vs context](decode_bottleneck_profiling_results/charts_no_hadacore/speedup_vs_context_by_batch.png)

![Latency vs batch](decode_bottleneck_profiling_results/charts_no_hadacore/latency_vs_batch_by_context.png)
...
![rocprof kernel categories](decode_bottleneck_profiling_results/charts_no_hadacore/rocprof_kernel_category_stacked.png)
...
![Projection GEMM roofline](decode_bottleneck_profiling_results/charts_no_hadacore/projection_gemm_roofline.png)
...
目前 unfused_INT4 的主要瓶頸是 PyTorch reference Hadamard/quant/dequant、完整 paged-cache dequant與大量 dispatch；current fusion 移除這些中間步驟後，主要成本轉為 QKV/O/MLP/LM-head GEMM、剩餘 PyTorch elementwise/copy，以及 host launch gap。長 context 下 attention占比提高，但 K1/K2/K3/FFN fused kernel 本身已不是最大 kernel-time項。
