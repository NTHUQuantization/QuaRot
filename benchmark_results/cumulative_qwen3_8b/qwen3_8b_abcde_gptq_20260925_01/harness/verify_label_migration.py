...
def verify(root):
    ...
    preserved=root/'source_migration_20260926/previous_schema3/runtime_adapter.py'
    label_only_adapter=preserved if preserved.exists() else root/'runtime_adapter.py'
    after=ast.parse(label_only_adapter.read_text())
    ...
    return {metadata['old_adapter_sha256'],digest(label_only_adapter)}
