...
NAMES = dict(A='HIP', B='GEMM', C='Hadacore', D='Fusion', E='PARD2')
...
def main(stage='formal'):
    ...
    for v in 'ABCDE':
        ...
        if (ROOT/'source_lock.json').exists():
            lock=json.loads((ROOT/'source_lock.json').read_text())['variants'][v]
            assert m['source_commit']==lock['commit'], 'Wrong source revision in result'
            assert m['source_sha256']==lock['source_sha256'], 'Wrong source/extension in result'
            if v in 'BC':
                assert m['adapter_sha256']==json.loads((ROOT/'source_migration_20260926/manifest.json').read_text())['adapter_after_sha256']
