# 本輪 GPTQ 覆寫規範（優先於下方歷史規範）

使用者於 2026-09-25 明確選定 A–E 全部共用 GPTQ，另開新一輪。

- run_id：qwen3_8b_abcde_gptq_20260925_01；與所有 RTN 結果分開。
- 共用 target：qwen3_8b_fused_v1_gptq_w4a4kv4_v1，GPTQ W4A4KV4；grouped_h256_v1、activation clip=0.9。這些是新 checkpoint 的數值格式，與舊 8B legacy RTN 不同；新舊耗時不混算。
- GPTQ：WikiText-2 train、128 samples、seqlen=2048、seed=0、w_groupsize=-1、symmetric INT4、w_clip=true、percdamp=0.01、act_order=false。
- 8B、14B、32B 的 checkpoint 都須通過 GPTQ config／實際 shard metadata 稽核；本輪正式效能測量範圍仍為 8B。
- 保留指定分支 kernel；ABC 只適配載入欄位與 cache API，選用分支原有 grouped_h256_v1 路徑。
- 報表順序 A HIP→B GEMM→C Hadacore→D Fusion→E PARD2。新版 B/C 在 A formal 完成後重跑 pilot，再依 B/C/E 完成 formal；A/D/E 的既有 pilot 保留，旧 B/C pilot 封存。
- 沿用原 96 正式／12 pilot prompts、input IDs、3 sweeps 順序、自然 EOS、cap、計時邊界、PARD2-TI k=15；drafter 保持原 BF16。
- p10/p90/p99、allocated/reserved peak memory、輸出差異與長度均保存。
- 正式結果入口 benchmark_results/cumulative_qwen3_8b/；工作檔位於 benchmark_work/，不混入正式報表。

---

# Qwen3-8B 累加消融：測速與呈現規範

日期：2026-09-26；版本 4（A=HIP、B=GEMM、C=Hadacore、D=Fusion、E=PARD2）。用途：交接給實作 benchmark、執行實驗與整理結果的 agent。
...
- 最初四組 C（Fusion）→現行 D、D（PARD2）→現行 E。schema 3 曾使用 B=舊 Hadacore、C=舊 GEMM；2026-09-26 組員更改分支實作後，本次更換為 B=新版 GEMM、C=新版 Hadacore。這次是來源更新，不能用舊 B/C 數據直接改標代替重測。
...
消融敘事與現行報表固定依 A→B→C→D→E 呈現。來源稽核確認 A/B 的 Hadamard 相同，B/C 的 GEMM／Linear4bit 相同；因此 A→B 加入 GEMM 優化，B→C 加入 HadaCore，最後 Fusion 與 PARD2。

| 版本 | 英文名稱 | 固定來源 | 本步重點 |
| --- | --- | --- | --- |
| A | HIP | `NTHUQuantization/QuaRot_Version`：`main` | HIP QuaRot INT4 基準；呈現名稱由 main 改為 HIP |
| B | GEMM | 同 repository：`GEMM`，`973a13dd849cbf2cee1e134919356ce71e23cdb5` | 僅 GEMM 優化；保留 HIP Hadamard |
| C | Hadacore | 同 repository：`Hadacore`，`6c30374901038138ac5094e6eaed35a5ca35db2f` | 保留 B 的 GEMM，再加入 HadaCore（GEMM+Hadacore） |
| D | Fusion | `NTHUQuantization/QuaRot`：`fused_pard2`，AR mode | 融合 runtime |
| E | PARD2 | 同一 fused runtime，PARD2-TI mode | 投機解碼 |

- A→B 對應 GEMM，B→C 對應 Hadacore，C→D 對應 Fusion，D→E 對應 PARD2。
- `main` 保留為真實 source branch 名稱，不把 branch 改名誤寫成來源 Git 操作；圖表與版本名稱使用 HIP。
- 各版若生成內容或長度不同，配對耗時比不能解讀成等輸出的純 runtime 加速。D→E 須獨立驗證等輸出。
- 本次更換 B/C 的來源 commit，須重新建置 extension 並重測 pilot／formal。A/D/E 的來源不變，共同 GPTQ 權重、prompt、orders、worker 與 adapter 計算邏輯維持不變。
- A 正式測量持續執行。A 結束後，先測新版 B/C pilot、驗證新來源與數值，再測 B/C/E formal；D formal 已完成。舊 B/C pilot 和舊合併報表封存於 `benchmark_work/qwen3_8b_abcde_gptq_20260925_01/source_migration_20260926/`，不納入現行表格。
- 歷史檔案明列其 label schema 與 source commit，保留當時的原始數值與命名。schema 3 的純改標證據和 schema 4 的來源更換證據分開保存，不能把歷史輸出歸給新版來源。
...
| B：GEMM | 待測 | 待測 | 待測 | 待測 | 待測 | 待測 |
| C：Hadacore | 待測 | 待測 | 待測 | 待測 | 待測 | 待測 |
