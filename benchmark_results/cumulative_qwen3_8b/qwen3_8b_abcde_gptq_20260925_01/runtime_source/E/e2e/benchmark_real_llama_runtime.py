...
def measure(workload, warmup, repeats, prepare=None):
    ...
    if prepare is not None:
        return measure_prepared(workload, prepare, warmup, repeats)
...
def measure_prepared(workload, prepare, warmup, repeats):
    """Use the same timer/statistics, excluding per-sample decode preparation.

    A speculative decoder consumes its prepared state. Prepare a fresh prefix
    before each sample, then reset memory counters so prefill allocations do
    not contaminate the decode peak. Resident prefix/cache memory is included.
    """
    for _ in range(warmup):
        prepare()
        workload()
    torch.cuda.synchronize()
    samples, peaks, baselines = [], [], []
    for _ in range(repeats):
        prepare()
        torch.cuda.synchronize()
    ...
    result = summarize(samples)
    result.update(baseline_memory_bytes=baselines[0], peak_memory_bytes=max(peaks),
                  incremental_peak_memory_bytes=max(p - b for p, b in zip(peaks, baselines)),
                  baseline_memory_bytes_per_sample=baselines,
                  peak_memory_bytes_per_sample=peaks)
    return result
...
def make_prefill(model, prompt, max_length):
    def workload():
        ...
        model(prompt, use_cache=True, logits_to_keep=1)
    return workload
...
def make_decode(model, prompt, next_token, max_length):
    model._expected_max_length = max_length
    output = model(prompt, use_cache=True, logits_to_keep=1)
    ...
    def workload():
        ...
        for _ in range(max_length - prompt.shape[1]):
            model(next_token, past_key_values=cache, use_cache=True, logits_to_keep=1)
    return workload
...
def parse_args(description=None, default_output="benchmark_real_llama_runtime.json"):
    ...
    parser.add_argument("--mode", choices=("fixed", "ar", "pard2-ti", "pard2-td"),
                        default="fixed", help="fixed preserves the original workload; other modes use a shared phased generation adapter")
    parser.add_argument("--draft-model", help="Pinned local PARD2 draft snapshot for phased generation")
    parser.add_argument("--tokenizer", help="Local tokenizer for phased generation")
    parser.add_argument("--benchmark-profile", choices=("qwen3_8b", "qwen3_14b", "qwen3_32b"),
                        default="qwen3_8b", help="Validate the selected target/drafter architecture")
    parser.add_argument("--input-ids", help="Optional JSON [batch, tokens] for equal-length phased workloads")
    parser.add_argument("--ti-zero-accept-fallback", type=int, default=0,
                        help="Opt-in TI+AR hybrid if the first N rounds accept no tokens; 0 keeps pure TI")
    parser.add_argument("--draft-prefill-logits", type=int, choices=(0, 1), default=1,
                        help="1 projects only final draft prefix logits; 0 retains all for ablation")
    ...
    if args.ti_zero_accept_fallback < 0 or (args.ti_zero_accept_fallback
            and (args.mode != 'pard2-ti' or args.batch_size != 1)):
        parser.error("--ti-zero-accept-fallback requires nonnegative N, --mode pard2-ti and batch=1")
    return args
...
def main(int4_loader=load_real_int4_model, fp16_loader=load_fp16_model,
    ...
    if args.mode != "fixed":
        from e2e.benchmark_pard2_phases import run_phases
