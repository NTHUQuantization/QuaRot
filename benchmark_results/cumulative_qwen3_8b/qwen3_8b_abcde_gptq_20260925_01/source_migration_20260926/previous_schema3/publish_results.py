...
REPOS={'A':WORKSPACE/'cumulative_ablation_abcde_20260925/upstream_repo','B':WORKSPACE/'cumulative_ablation_abcde_20260925/branch_Hadacore','C':WORKSPACE/'cumulative_ablation_abcde_20260925/branch_GEMM',
       'D':WORKSPACE/'fused_v1','E':WORKSPACE/'fused_v1'}
...
def audit():
    from verify_label_migration import verify
    ...
    final=dict(valid=True,finished_unix=time.time(),weight_files=verified,
               ...
               records=4320,pilot_records=540,label_schema_version=3,label_only_adapter_change_verified=True)
...
def package(variants):
    ...
    for name in ('protocol_manifest.json','variant_mapping.json','source_audit.json',
                 'host_git_metadata.json','checkpoint_preflight.json','final_audit.json','environment.json'):
        copy(ROOT/name,out/name)
    copy(ROOT/'protocol_zh.md',out/'protocol_zh.md')
    for name in ('measure.py','runtime_adapter.py','replay.py','summarize.py','publish_results.py','run_queue.py','test_summary.py','initialize.py','verify_label_migration.py'):
...
Read [the full report](formal/report_zh.md). A=HIP (source branch main), B=Hadacore,
C=GEMM, D=Fusion, E=PARD2 (TI).
The ablation narrative is HIP -> Hadacore -> GEMM -> Fusion -> PARD2.
B adds HadaCore while retaining HIP GEMM; the newer C retains HadaCore and
adds the optimized GEMM. Source branch names and commits remain traceable.
...
def main():
    ...
    text=index_readme.read_text().replace('A–E，待量化與完整測量驗證','A–E，已完成驗證與遠端推送')
