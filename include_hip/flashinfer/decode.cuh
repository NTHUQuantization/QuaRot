...
template<
  ...
  typename IdType
>
__global__ void BatchDecodeWithPagedKVGQAKernel(
  DTypeInQ *__restrict__ q,
  paged_kv_t<DTypeIn, IdType> paged_kv,
  DTypeOut *__restrict__ o,
  size_t num_q_heads,
  ...
  if (q_head_idx >= num_q_heads || kv_head_idx >= num_kv_heads || num_q_heads % num_kv_heads != 0) {
    return;
  }
...
#pragma unroll
  ...
  size_t producer_entry_base = 0, stage_idx = 0;
  constexpr size_t vec_bits = quant::size_of_type<DTypeIn>() * vec_size * 8 * FoldFactor;
  static_assert(vec_bits == 128, "We want to highly utilize the memory bandwidth");
...
hipError_t SingleDecodeWithKVCache(
  ...
  hipStream_t stream = nullptr, size_t dev_id = 0,
  ...
  FLASHINFER_CUDA_CALL(hipSetDevice(dev_id));

  SWITCH_HEAD_DIM(
      head_dim, HEAD_DIM,
      {SWITCH_ROTARY_MODE(
          rotary_mode, ROTARY_MODE, {SWITCH_LAYOUT(layout, QKV_LAYOUT, {
            constexpr size_t vec_size = ::max(static_cast<size_t>(16 / quant::size_of_type<DTypeIn>()), HEAD_DIM / 32) / 2;
            ...
            if (seq_len <= 128) {
              ...
              FLASHINFER_CUDA_CALL(
                  hipLaunchCooperativeKernel((void *)kernel, nblks, nthrs, args, 0, stream));
            }
          })})});
  return hipSuccess;
}
...
hipError_t BatchDecodeWithPagedKVCache(
  DTypeInQ *q,
  paged_kv_t<DTypeIn, IdType> paged_kv,
  DTypeOut *o,
  RotaryMode rotary_mode = RotaryMode::kNone,
  ...
  SWITCH_HEAD_DIM(
      paged_kv.head_dim, HEAD_DIM, {SWITCH_ROTARY_MODE(rotary_mode, ROTARY_MODE, {
        ...
        constexpr size_t vec_size = ::max(static_cast<size_t>(16 / quant::size_of_type<DTypeIn>() / FoldFactor), HEAD_DIM / 32);
        constexpr size_t bdx = HEAD_DIM / vec_size;
        constexpr size_t bdy = 128 / bdx;
