...
inline __attribute__((always_inline)) __device__ __host__ __hip_bfloat162 make_bfloat162( const __hip_bfloat16 x, const __hip_bfloat16 y );
...
template <size_t vec_size>
FLASHINFER_INLINE void cast_from_impl(const vec_t<__hip_bfloat16, vec_size> &src,
                                      vec_t<float, vec_size> &dst) {
  if constexpr (vec_size == 1) {
    dst.data = float(src.data);
  } else {
#pragma unroll
    for (size_t i = 0; i < vec_size / 2; ++i) {
      ((float2 *)(&dst.data))[i] = __bfloat1622float2(((__hip_bfloat162 *)(&src.data))[i]);
    }
  }
}
...
template <size_t vec_size>
FLASHINFER_INLINE void cast_from_impl(const vec_t<float, vec_size> &src,
                                      vec_t<__hip_bfloat16, vec_size> &dst) {
  if constexpr (vec_size == 1) {
    dst.data = __hip_bfloat16(src.data);
  } else {
#pragma unroll
    for (size_t i = 0; i < vec_size / 2; ++i) {
      ((__hip_bfloat162 *)(&dst.data))[i] = __float22bfloat162_rn(((float2 *)(&src.data))[i]);
    }
  }
}
...
template <size_t vec_size>
FLASHINFER_INLINE void cast_from_impl(const vec_t<__hip_fp8_e4m3_fnuz, vec_size> &src,
                                      vec_t<float, vec_size> &dst) {
  if constexpr (vec_size == 1) {
    dst.data = float(src.data);
  } else if constexpr (vec_size == 2) {
    *(float2 *)(&dst.data) = float2(*(__hip_fp8x2_e4m3_fnuz *)(&src.data));
  } else {
#pragma unroll
    for (size_t i = 0; i < vec_size / 4; ++i) {
      ((float4 *)(&dst.data))[i] = float4(((__hip_fp8x4_e4m3_fnuz *)(&src.data))[i]);
    }
  }
}
...
#pragma unroll
    for (size_t i = 0; i < vec_size / 2; ++i) {
      ((half2 *)(&dst.data))[i] = half2(((__hip_fp8x2_e4m3_fnuz *)(&src.data))[i]);
    }
  }
}
...
template <size_t vec_size>
FLASHINFER_INLINE void cast_from_impl(const vec_t<float, vec_size> &src,
                                      vec_t<__hip_fp8_e4m3_fnuz, vec_size> &dst) {
  if constexpr (vec_size == 1) {
    dst.data = __hip_fp8_e4m3_fnuz(src.data);
  } else if constexpr (vec_size == 2) {
    *(__hip_fp8x2_e4m3_fnuz *)(&dst.data) = __hip_fp8x2_e4m3_fnuz(*(float2 *)(&src.data));
  } else {
#pragma unroll
    for (size_t i = 0; i < vec_size / 4; ++i) {
      ((__hip_fp8x4_e4m3_fnuz *)(&dst.data))[i] = __hip_fp8x4_e4m3_fnuz(((float4 *)(&src.data))[i]);
    }
  }
}
...
FLASHINFER_INLINE void cast_from_impl(const vec_t<half, vec_size> &src,
  ...
  if constexpr (vec_size == 1) {
    ...
    *(__hip_fp8x2_e4m3_fnuz *)(&dst.data) = __hip_fp8x2_e4m3_fnuz(*(half2 *)(&src.data));
  } else {
#pragma unroll
    for (size_t i = 0; i < vec_size / 4; ++i) {
      // NOTE(Zihao): need to double check if we properly handle flo and fhi
      ((__hip_fp8x4_e4m3_fnuz *)(&dst.data))[i] =
          __hip_fp8x4_e4m3_fnuz(((half2 *)(&src.data))[i * 2], ((half2 *)(&src.data))[i * 2 + 1]);
    }
  }
}

template <size_t vec_size>
FLASHINFER_INLINE void cast_from_impl(const vec_t<__hip_fp8_e5m2_fnuz, vec_size> &src,
                                      vec_t<float, vec_size> &dst) {
  if constexpr (vec_size == 1) {
    dst.data = float(src.data);
  } else if constexpr (vec_size == 2) {
    *(float2 *)(&dst.data) = float2(*(__hip_fp8x2_e5m2_fnuz *)(&src.data));
  } else {
#pragma unroll
    for (size_t i = 0; i < vec_size / 4; ++i) {
      ((float4 *)(&dst.data))[i] = float4(((__hip_fp8x4_e5m2_fnuz *)(&src.data))[i]);
    }
  }
}
...
#pragma unroll
    for (size_t i = 0; i < vec_size / 2; ++i) {
      ((half2 *)(&dst.data))[i] = half2(((__hip_fp8x2_e5m2_fnuz *)(&src.data))[i]);
    }
  }
}
...
template <size_t vec_size>
FLASHINFER_INLINE void cast_from_impl(const vec_t<float, vec_size> &src,
                                      vec_t<__hip_fp8_e5m2_fnuz, vec_size> &dst) {
  if constexpr (vec_size == 1) {
    dst.data = __hip_fp8_e5m2_fnuz(src.data);
  } else if constexpr (vec_size == 2) {
    *(__hip_fp8x2_e5m2_fnuz *)(&dst.data) = __hip_fp8x2_e5m2_fnuz(*(float2 *)(&src.data));
  } else {
#pragma unroll
    for (size_t i = 0; i < vec_size / 4; ++i) {
      ((__hip_fp8x4_e5m2_fnuz *)(&dst.data))[i] = __hip_fp8x4_e5m2_fnuz(((float4 *)(&src.data))[i]);
    }
  }
}
...
FLASHINFER_INLINE void cast_from_impl(const vec_t<half, vec_size> &src,
  ...
  if constexpr (vec_size == 1) {
    ...
    *(__hip_fp8x2_e5m2_fnuz *)(&dst.data) = __hip_fp8x2_e5m2_fnuz(*(half2 *)(&src.data));
  } else {
#pragma unroll
    for (size_t i = 0; i < vec_size / 4; ++i) {
      // NOTE(Zihao): need to double check if we properly handle flo and fhi
      ((__hip_fp8x4_e5m2_fnuz *)(&dst.data))[i] =
          __hip_fp8x4_e5m2_fnuz(((half2 *)(&src.data))[i * 2], ((half2 *)(&src.data))[i * 2 + 1]);
    }
  }
}
